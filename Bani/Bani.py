...
class FAQWrapper:
    ...
    def solveForQuery(self, queryVector: np.ndarray, K: int, topSimilar: int = 5) -> FAQOutput:
        ...
        return FAQOutput(faqId=self.id, faqName=self.FAQ.name, answer=bestAnswer,
                         ...
                         similarQuestions=self._getClosestQuestions(rankedIndices, topSimilar, bestMatchQuestion.text),
                         maxScore=maxScore)
...
class Bani:
    ...
    def _registerFAQs(self, FAQs: List[FAQ]):
        ...
        for faq in FAQs:
            if (faq.isUsable() == False):
                raise ValueError(
                    "All faqs passed to chadBot must be Usable !!!! please build FAQ again or load from preexisting one")

        for faq in FAQs:
            if (faq.hasVectorsAssigned() == True and self.assignVectors == False):
                warnings.warn(
                    "Vectors already assigned to {} FAQ , if you want to reassign using the current model please clear the vectors using resetAssigned vectors".format(
                        faq.name))
            elif (faq.hasVectorsAssigned() == True and self.assignVectors == True):
                print(
                    "OverWritting the vectors of FAQ named {} , it will name save the generated vectors , to do that use the saveFAQ/s feature".format(
                        faq.name))
                faq._assignVectors(model=self.model)
            elif (faq.hasVectorsAssigned() == False and self.assignVectors == True):
                print(
                    "Assigning vectors to {} faq , , it will name save the generated vectors , to do that use the saveFAQ/s feature".format(
                        faq.name))
                faq._assignVectors(model=self.model)
            else:
                raise VectorNotAssignedException(
                    "assignVectors is False , but the vectors are not stored in faq named {}".format(faq.name))
    ...
    def train(self, outputPath: str, batchSize=16, epochs: int = 1, lossName: str = "batchHardTriplet", **kwargs):
        ...
        self.model.fit(train_objectives=trainingObjectives, epochs=epochs, save_best_model=False,
